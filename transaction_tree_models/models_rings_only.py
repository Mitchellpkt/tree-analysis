...
class Analysis(BaseModel):
    ...
    def ingest_ring_and_update_self(self, ring: Ring) -> None:
        """ Processes and records a ring """
        if ring not in self.rings:
            # Add the ring to our list if not already known
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        # Ingest all the rings when initialized
        for ring in self.rings:
            self.ingest_ring_and_update_self(ring)
